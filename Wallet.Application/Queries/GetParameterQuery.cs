...
public class GetParameterQueryHandler(IUnityOfWork unityOfWork, IMapper mapper, IValidator validator) : IRequestHandler<GetParameterQuery, BaseResponse<GetParameterOutput>>
{
    public async Task<BaseResponse<GetParameterOutput>> Handle(GetParameterQuery request, CancellationToken cancellationToken)
    {
        var parameter = await unityOfWork.GetRepository<Domain.Entities.Parameter>()
